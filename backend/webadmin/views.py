...
def vigile_cv_pdf_view(request, pk):
    ...
    try:
        ...
        pdf_bytes = build_vigile_cv_pdf(vigile)
    except ImportError:
        messages.error(
            request,
        ...
        _logger.exception("Erreur génération CV PDF vigile %s", pk)
        messages.error(request, "Impossible de générer le CV PDF. Réessayez ou contactez le support.")
        return redirect(reverse("webadmin-vigile-detail", args=[pk]))
