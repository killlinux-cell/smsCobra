...
from accounts.serializers_admin import (
    ControllerAdminSerializer,
    ControllerCreateSerializer,
    ControllerUpdateSerializer,
...
class AdminControllerListCreateView(generics.ListCreateAPIView):
    """Liste des contrôleurs + création (multipart, comme le web)."""
    ...
    def get_serializer_class(self):
        if self.request.method == "POST":
            return ControllerCreateSerializer
        return ControllerAdminSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        ser.is_valid(raise_exception=True)
        user = ser.save()
        out = ControllerAdminSerializer(user, context={"request": request})
        return Response(out.data, status=status.HTTP_201_CREATED)


class AdminControllerDetailView(generics.RetrieveUpdateAPIView):
    """Fiche contrôleur + mise à jour."""
    ...
    def get_queryset(self):
        return User.objects.filter(role=User.Role.CONTROLEUR).prefetch_related(
            "controller_site_assignments__site"
        )

    def get_serializer_class(self):
        if self.request.method in ("PUT", "PATCH"):
            return ControllerUpdateSerializer
        return ControllerAdminSerializer
